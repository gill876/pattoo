...
from pattoo.constants import (
    PATTOO_API_PORTAL_NAME, PATTOO_API_PORTAL_PROXY)
from pattoo.configuration import ConfigPortal as Config
from pattoo.api.portal import PATTOO_PORTAL
...
def main():
    ...
    agent_gunicorn = Agent(PATTOO_API_PORTAL_PROXY, config=config)

    # Create agent for daemon
    config = Config()
    agent_api = AgentAPI(
        PATTOO_API_PORTAL_NAME,
        PATTOO_API_PORTAL_PROXY,
        PATTOO_PORTAL,
        config=config)
